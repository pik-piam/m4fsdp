...
spatialMapsAllFSMDiffmap <- function(repReg, repIso, repGrid, reg2iso, file = NULL, subset=c("FSTsdp_minus_BASEssp2")) {
  ...
  row3 <- ggplotify::as.grob(wrap_plots(wrap_elements(group3),wrap_elements(group4),ncol = 2, nrow = 1, widths=c(0.665,0.335)))

  ggsave(paste0(file_base,"_combined",".png"),width=10, height=14.1, scale= 1.7,
         marrangeGrob(grobs = list(row1,row2,row3), heights= c(0.222,0.383,0.396), nrow=3, ncol=1,top=NULL))
  ggsave(paste0(file_base,"_combined",".pdf"),width=10, height=14.1, scale= 1.7,
         marrangeGrob(grobs = list(row1,row2,row3), heights= c(0.222,0.383,0.396), nrow=3, ncol=1,top=NULL))
}
