...
#' FSEC-relevant results, and saves them to each scenarios' respective report.mif.
#'
#' @export
#' 
#' @param healthImpacts_filePath full pathname of the health impacts dataset
...
integrateHealthImpacts <- function(healthImpacts_filePath, scenario, dir = ".") {
    
    # -----------------------------------------------------------------------------------------------------------------
    # Format Marco's health impacts dataset
    
    gdx <- suppressWarnings(readGDX(healthImpacts_filePath))[[2]]
    getSets(gdx) <- c("region", "year", "scenario", "unit", "TMREL", "riskFactor", "causeOfDeath", "sex", "stat")
    ...
    colnames(healthImpacts) <- c("cell", "region", "year", "scenario",
                                 "unit", "TMREL", "riskFactor", "causeOfDeath", "sex", "stat", "value")
    ...
    # Append to country-level report_iso.rds
    ...
    if (file.exists(rds_file)) {
        ...
        if (any(healthImpacts$variable %in% report_iso_rds$variable)) {
            ...
            report_iso_rds <- report_iso_rds %>%
                filter(!(variable %in% healthImpacts$variable))
        }
